...
//#region node_modules/.nitro/vite/services/ssr/assets/puppet-ai-dPtHIkiG.js
...
async function geminiChat(messages, maxTokens = 2400, model = process.env.GEMINI_TEXT_MODEL || "gemini-3.8-flash") {
	try {
		const ai = getGeminiClient();
...
async function geminiImage(prompt, model = process.env.GEMINI_IMAGE_MODEL || "gemini-3.1-flash-lite-image") {
	try {
...
async function runExtractionPipeline(request, providerOverride) {
	...
	if (!candidates.length) {
		...
		throw new Error("All configured AI extraction providers failed");
	}
...
//#endregion
export { analyzeFigure_createServerFn_handler, composeAnimation_createServerFn_handler, conjureFigure_createServerFn_handler };
